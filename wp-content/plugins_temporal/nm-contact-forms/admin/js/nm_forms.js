function addField(element){
		...
		jQuery(element).closest('.nm_form').find('.nm_form_fields').prepend('<li class="nm_item menu-item menu-item-page">'
			...
			+'</td></tr>'
			+'<tr class="nm_read_only"><th>'
			+'Read-only:'  
			+'</th><td>'
			+'<input type="checkbox" name="nm_f['+nm_form_id+'][fields][nm_field_'+fields_counter+'][read_only]"/>'
			+'</td></tr>'
...
jQuery(document).ready(function(){
			...
			+'<li class="nm_item menu-item menu-item-page">'
				...
				+'<div class="nm_field_settings" style="display:block;">'
								...
								+'<table class="nm_table">'
									...
									+'Reply-to email:'
									+'</th><td>'
										+'<input type="text" name="nm_f['+ nm_form_id +'][reply_to]" value="">'
	...
	jQuery(document).on( 'change', '.nm_field_type',function(event){
		...
		if(option == 'select' || option == 'radio' || option == 'checkbox'){
			...
			jQuery(this).closest('.nm_table').find('.nm_exts,.nm_size,.nm_placeholder,.nm_html,.nm_read_only').hide();
			...
			jQuery(this).closest('.nm_table').find('.nm_select_options,.nm_required,.nm_get,.nm_placeholder,.nm_exts,.nm_size,.nm_html,.nm_read_only').hide();
		}else if(option == 'file_upload'){	
		
			jQuery(this).closest('.nm_table').find('.nm_exts,.nm_size').show();
			jQuery(this).closest('.nm_table').find('.nm_select_options,.nm_get,.nm_placeholder,.nm_html,.nm_read_only').hide();
			...
			jQuery(this).closest('.nm_table').find('.nm_select_options,.nm_required,.nm_placeholder,.nm_exts,.nm_size,.nm_html,.nm_read_only').hide();
		
		}else if(option == 'text' || option == 'textarea' || option == 'email'){
		
			jQuery(this).closest('.nm_table').find('.nm_required,.nm_placeholder,.nm_get,.nm_read_only').fadeIn(500);
			...
			jQuery(this).closest('.nm_table').find('.nm_select_options,.nm_exts,.nm_size,.nm_get,.nm_required,.nm_placeholder,.nm_read_only').hide();
			...
			jQuery(this).closest('.nm_table').find('.nm_select_options,.nm_get,.nm_placeholder,.nm_required,.nm_exts,.nm_size,.nm_html,.nm_read_only').hide();
